...
public class SorterImpl<E> implements Sorter<E> {
    ...
    public List<E> selInsSort(List<E> items, Comparator<E> comparator) {
        //insertion method since it's stable
        for (int i = 0; i < items.size(); i++) {
            E placeHolder = items.get(i);
            int j = i;

            // Move elements of items[0..i-1], that are greater than placeholder get moved up 1+
            while (j > 0 && comparator.compare(items.get(j - 1), placeHolder) > 0) {
    ...
    private void quickSortPart(List<E> items, int from, int to, Comparator<E> comparator) {
        //make sure items is not empty/null etc
        if (items == null || items.size() == 0)
            return;
        //make sure the start and end of index make sense
        if (from >= to)
            return;

        //get the index to separate the two sides from
    ...
    //this is an added method to support quickSortPart, almost the same as the example in the slides
    private int partition(List<E> items, int from, int to, Comparator<E> comparator) {
        ...
        do {
            //skip lower variables that are left
            while (bottom <= top && comparator.compare(items.get(bottom), pivot) < 0) bottom++;
            //skip higher variables that are right
            while (bottom <= top && comparator.compare(pivot, items.get(top)) < 0) top--;

            //if top is higher/equal than bottom, set top to bottom index and top to temp
            // bottom/top +/- to shrink list
            if (top >= bottom) {
                E temp = items.get(bottom);
                items.set(bottom, items.get(top));
                items.set(top, temp);
                bottom++;
                top--;
            }
    ...
    public List<E> topsHeapSort(int numTops, List<E> items, Comparator<E> comparator) {
        ...
        for (int i = numTops; i < items.size(); i++) {
            ...
            //  the root of the heap is the currently trailing placeHolder in the lead collection,
            //  which will lose its membership if a better placeHolder is found from position i onwards
            E placeHolder = items.get(i);
            E worstLeadItem = items.get(0);
            if (comparator.compare(placeHolder, worstLeadItem) < 0) {
                // placeHolder < worstLeadItem, so shall be included in the lead collection
                items.set(0, placeHolder);
                // demote worstLeadItem back to the tail collection, at the orginal position of placeHolder
...
//         quickSortPart(items, 0, numTops-1, comparator);
    ...
    private void heapSwim(List<E> items, int heapSize, Comparator<E> comparator) {
        //start the swim at heapSize - 1;
        int cIndex = heapSize - 1;
        int pIndex = (cIndex - 1) / 2;

        E swimmer = items.get(cIndex);
        //find lowest pIndex
        while (pIndex >= 0 && comparator.compare(swimmer, items.get(pIndex)) < 0) {
            //swap
            Collections.swap(items, cIndex, pIndex);

            cIndex = pIndex;
            pIndex = (cIndex - 1) / 2;
        }
        items.set(cIndex, swimmer);
    }
    ...
    private void heapSink(List<E> items, int heapSize, Comparator<E> comparator) {
        ...
        int pIndex = 0;
//object to sink
        E down = items.get(pIndex);

        while (cIndex <= (heapSize - 1)) {
            E placeHolder = items.get(cIndex);

            //find lowest cIndex+1
            if ((cIndex + 1) <= (heapSize - 1) && comparator.compare(items.get(cIndex + 1), placeHolder) < 0) {
                cIndex++;
                placeHolder = items.get(cIndex);
            }
            //see if the bottom has been reached
            if (comparator.compare(down, placeHolder) <= 0) break;
            items.set(pIndex, placeHolder);
            pIndex = cIndex;
            cIndex = (pIndex * 2) + 1;
        }
        items.set(pIndex, down);
    }
}
