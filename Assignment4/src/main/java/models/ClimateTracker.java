...
public class ClimateTracker {
    ...
    private final Map<Integer, Station> stations;        // all available weather stations organised by Station Number (STN)


    public Set<Station> getStations() {
        return Set.copyOf(new LinkedHashSet<>(stations.values()));
    }
     ...
     *
     * @return a map of Stations and their amount of measurements
     */
    public Map<Station, Integer> numberOfMeasurementsByStation() {
        ...
        //stream through the values of stations, collect the station as key and get the measurements.size as value.
        //Then we order the map as a LinkedHashMap to keep the order of insertion.
        return new LinkedHashMap<>(stations.values()
                .stream()
                .collect(Collectors.toMap(k -> k, v -> v.getMeasurements().size(), (o1, o2) -> o1, LinkedHashMap::new)));
    }
...
//stream through the values of stations, filter all the present firstdayMeasurements, collect these into a map
        // collect the stations as keys and the first day of measurements as values, sort these into a new Linkedhashmap to maintain order.
        return stations.values()
                .stream()
                .filter(m -> m.firstDayOfMeasurement().isPresent())
                .collect(Collectors.toMap(k -> k, s -> s.firstDayOfMeasurement().orElseThrow(), (o1, o2) -> o1, LinkedHashMap::new));
    }
    ...
    public Map<Station, Integer> numberOfValidValuesByStation(Function<Measurement, Double> mapper) {
        ...
        return stations.values()
                .stream()
                .collect(Collectors.toMap(k -> k, v -> v.numValidValues(mapper), (o1, o2) -> o1, LinkedHashMap::new));
    }
...
//First we stream through all the stations, we flatten the measurements and stream through those
// Next filter the measurement to check for viable data
// afterwards we collect this data and  group it by the year as it's key and the average of all the AverageTemperature data for the year
        return stations.values()
                .stream()
                .flatMap(s ->
                        s.getMeasurements().stream())
                .filter(measurement ->
                        !Double.isNaN(measurement.getAverageTemperature()))
...
//Get all stations together, stream through them, then combine all their data
        //filter data to find viable data, collect that into map with year as key and the max of any annual dataset.
    ...
    public Map<Month, Double> allTimeAverageDailySolarByMonth() {
        ...
        return getStations()
                .stream()
                .flatMap(s -> s.getMeasurements()
                        .stream()
                        .filter(m -> !Double.isNaN(m.getSolarHours())))
                .collect(Collectors.groupingBy(m -> Month.of(m.getDate().getMonthValue()), TreeMap::new, Collectors.averagingDouble(Measurement::getSolarHours)));
    }
    ...
    public int coldestYear() {
        return getStations()
                .stream()
                .flatMap(s -> s.getMeasurements().stream()
                        .filter(m -> !Double.isNaN(m.getMinTemperature()) && m.getMinTemperature() <= 0))
                .collect(Collectors.groupingBy(m -> m.getDate().getYear(), Collectors.summingDouble(Measurement::getMinTemperature)))
                .entrySet().stream()
